...
@section('dashboard-content')
    <div class="flex-1 p-8">
        ...
        <!-- Search -->
        <form method="GET" action="{{ route('admin.categories.index') }}" class="py-4 flex gap-3 items-center">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center gap-2">
                                        ...
                                        <form action="{{ route('admin.categories.destroy', $category->id) }}"
                                            method="POST" onsubmit="return confirm('Delete this category? (all data related to this category will be deleted too)')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
    ...
    @if (session('open_edit') && old('category_id'))
        <script>
            document.addEventListener('DOMContentLoaded', () => {
